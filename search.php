<?php
	...
	$search = isset($_GET['s']) ? $_GET['s'] : "popular";
?>
<!DOCTYPE html>
<html lang="en-US">
<head>
	...
	<title>Search Result For <?=$search?> Movies and TV Show <?php if ($page>=2) { echo "- Page " . $page . " "; } ?>- <?=TITLE_SITE?></title>
							...
							<div class="row">
								<div class="col-md-12">
									<ul class="breadcrumb" id="crumbs" typeof="BreadcrumbList" vocab="http://schema.org/">
										...
										<li class="active">Search Results for "<?=$search?>" Query</li>
									</ul>
								</div>
							</div>
								...
								<div class="col-md-12">
									...
									<div class="tab-content">
										<div class="tab-pane fade in active" id="movies" role="tabpanel">
											...
											if (false !== ($html = @file_get_contents($search_url))) {	
												...
												foreach ($search_movie['results'] as $result) {
													...
													if (SEO_FRIENDLY=="yes") { 
														if (SHOW_TITLE_PARAMETER=="yes") {
															$seoFriendlySrcMv = $_SERVER['HTTP_HOST'] . "/movie/".$search_movie_id."/".slugUri($title); 
														} else {
															$seoFriendlySrcMv = $_SERVER['HTTP_HOST'] . "/movie/".$search_movie_id; 
														}
													} else {
														if (SHOW_TITLE_PARAMETER=="yes") {
															$seoFriendlySrcMv = $_SERVER['HTTP_HOST'].'/'.FILE_NAME.'?movie='.$search_movie_id.'&title='.slugUri($title);
														} else {
															$seoFriendlySrcMv = $_SERVER['HTTP_HOST'].'/'.FILE_NAME.'?movie='.$search_movie_id;
														}
													}
													echo '
														<article class="media post-14070" id="post-14070">
															<div class="media-left">
																<a href="//'.$seoFriendlySrcMv.'" target="_blank"><img alt="'.$title.'" height="138" src="'.$poster_path.'" width="92"></a>
															</div>


															<div class="media-body">
																<header class="entry-header">
																	<h2 class="entry-title"><a href="//'.$seoFriendlySrcMv.'" target="_blank" rel="bookmark">'.$title.'</a> <span class="text-color">'.$year.'</span></h2>
												...
												<nav>
													<ul class="pagination">
														...
														<?php } if (($page==1) || ($page==2)) { ?>
															<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/search.php?s=<?=urlencode($search)?>"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
														<?php } if ($page>=3) { ?>
															<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/search.php?s=<?=urlencode($search)?>&page=<?=$page-1?>"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
														<?php } ?>
														...
														if ($total_results>=20) {
															...
															if ($total_pages >=1 && $page <= $total_pages) {
																...
																$pagination = "";
																if ($page > ($limit/2)) { 
																	$pagination .= '<li><a aria-label="First" href="//'.$_SERVER['HTTP_HOST'].'/search.php?s='.urlencode($search).'">1</a></li>
																...
																if ($count < $limit)
																	if ($x==1) {
																		...
																		$pagination .= "<li><a href=\"//".$_SERVER['HTTP_HOST']."/search.php?s=".urlencode($search)."&page=".$x."\">".$x." </a></li>";
																	}
																	$count++;
																}
																if ($page < $total_pages - ($limit/2)) { 
																	$pagination .= '<li><span><span aria-hidden="true">...</span></span></li><li><a href="//'.$_SERVER['HTTP_HOST'].'/search.php?s='.urlencode($search).'&page='.$total_pages.'">'.$total_pages.'</a></li><li><a aria-label="Next" href="//'.$_SERVER['HTTP_HOST'].'/search.php?s='.urlencode($search).'&page='.($page+1).'"><span class="hidden-xs">Next</span> &rsaquo;</a></li>'; 
																}
															}
															echo $pagination;
														}
										...
										<div class="tab-pane" id="tvshow" role="tabpanel">
											...
											$total_results = $search_tv['total_results'];
												foreach ($search_tv['results'] as $result_tv) {
													$poster_path_tv = isset($result_tv['poster_path']) ? "//image.tmdb.org/t/p/w92".$result_tv['poster_path'] : "//".$_SERVER['HTTP_HOST']."/assets/img/no_poster_available.png";
													$overview_tv = $result_tv['overview'];
													$release_date_tv = isset($result_tv['release_date']) ? $result_tv['release_date'] : $result_tv['first_air_date'];
													$year_tv = @current(explode('-', $release_date));

													$genre_ids_tv = @$result_tv['genre_ids'];
													$arr_genre_tv = count($genre_ids_tv);

													$search_tv_id = $result_tv['id'];
													$title_tv = isset($result_tv['title']) ? $result_tv['title'] : $result_tv['name'];
													$backdrop_path_tv = $result_tv['backdrop_path'];
													$popularity_tv = $result_tv['popularity'];
													$vote_count_tv = $result_tv['vote_count'];
													$vote_average_all_tv = $result_tv['vote_average'];
													...
													if (SEO_FRIENDLY=="yes") { 
														if (SHOW_TITLE_PARAMETER=="yes") {
															$seoFriendlySrcTv = $_SERVER['HTTP_HOST'] . "/tv/".$search_tv_id."/".slugUri($title_tv); 
														} else {
															$seoFriendlySrcTv = $_SERVER['HTTP_HOST'] . "/tv/".$search_tv_id; 
														}
													} else {
														if (SHOW_TITLE_PARAMETER=="yes") {
															$seoFriendlySrcTv = $_SERVER['HTTP_HOST'].'/'.FILE_NAME.'?tv='.$search_tv_id.'&title='.slugUri($title_tv);
														} else {
															$seoFriendlySrcTv = $_SERVER['HTTP_HOST'].'/'.FILE_NAME.'?tv='.$search_tv_id;	
														}
													}
													echo '
														<article class="media post-14070" id="post-14070">
															<div class="media-left">
																<a href="//'.$seoFriendlySrcTv.'" target="_blank"><img alt="'.$title_tv.'" height="138" src="'.$poster_path_tv.'" width="92"></a>
															</div>


															<div class="media-body">
																<header class="entry-header">
																	<h2 class="entry-title"><a href="//'.$seoFriendlySrcTv.'" target="_blank" rel="bookmark">'.$title_tv.'</a> <span class="text-color">'.$year_tv.'</span></h2>

																	<div class="entry-meta">
																		<div class="rating" data-placement="right" data-toggle="tooltip" title="'.$vote_average_all_tv.'">'; ?>
																		<?php
																		if ($vote_average_tv==10) {
																			echo '
																			<div class="rating-symbol glyphicon glyphicon-star on"></div>
																			<div class="rating-symbol glyphicon glyphicon-star on"></div>
																			<div class="rating-symbol glyphicon glyphicon-star on"></div>
																...
																<footer class="entry-footer">'; ?> 
																	<?php 
																		$separator = ''; for($x = 0; $x < $arr_genre_tv; $x++) {
																			echo $separator; echo '<a href="//' . $_SERVER['HTTP_HOST'] . '/genre-tv/' .  $genre_ids_tv[$x] . '/' . slugUri(str_replace(array("28", "12", "16", "35", "80", "99", "18", "10751", "14", "36", "27", "10402", "9648", "10749", "878", "10770", "53", "10752", "37", "10759", "10762", "10763", "10764", "10765", "10766", "10767", "10768"), array("Action", "Adventure", "Animation", "Comedy", "Crime", "Documentary", "Drama", "Family", "Fantasy", "History", "Horror", "Music", "Mystery", "Romance", "Science Fiction", "TV Movie", "Thriller", "War", "Western", "Action & Adventure", "Kids", "News", "Reality", "Sci-Fi & Fantasy", "Soap", "Talk", "War & Politics"), $genre_ids_tv[$x])) . '" target="_blank">' . str_replace(array("28", "12", "16", "35", "80", "99", "18", "10751", "14", "36", "27", "10402", "9648", "10749", "878", "10770", "53", "10752", "37", "10759", "10762", "10763", "10764", "10765", "10766", "10767", "10768"), array("Action", "Adventure", "Animation", "Comedy", "Crime", "Documentary", "Drama", "Family", "Fantasy", "History", "Horror", "Music", "Mystery", "Romance", "Science Fiction", "TV Movie", "Thriller", "War", "Western", "Action & Adventure", "Kids", "News", "Reality", "Sci-Fi & Fantasy", "Soap", "Talk", "War & Politics"), $genre_ids_tv[$x]) . '</a>'; if (!$separator) $separator = ' | '; 
																		} 
																	?>
																<?php echo '</footer>
																<!-- .entry-footer -->
