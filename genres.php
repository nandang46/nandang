<?php
	...
	$genreid = isset($_GET['genremovie']) ? $_GET['genremovie'] : $_GET['genretv'];
	//$genretv = isset($_GET['genretv']) ? $_GET['genretv'] : NULL;
	$page = isset($_GET['page']) ? $_GET['page'] : 1;
	$genre_title = ucwords(str_replace(array("-", "and"), array(" ", "&"), $_GET['title']));
	if (!empty($_GET['genremovie'])) { $type = "movie"; } else { $type = "tv"; }
?>
<!DOCTYPE html>
<html lang="en-US">
<head>
	...
	<title><?=str_replace("Tv", "TV", $genre_title)?> Archives <?php if ($page>=2) { echo "- Page " . $page . " "; } ?>- <?=TITLE_SITE?></title>
	<meta name="author" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="copyright" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="description" content="All <?=$type?> lists from <?=$genre_title?> archives <?php if ($page>=2) { echo "page " . $page . " "; } ?>">
					...
					<div class="content-area col-md-8" id="primary">
						<main class="site-main" id="main" role="main">
							<div class="row">
								<div class="col-md-12">
									<ul class="breadcrumb" id="crumbs" typeof="BreadcrumbList" vocab="http://schema.org/">
										...
										<li class="active">Archive for <?=$genre_title?></li>
									</ul>
								</div>
							</div>
							...
							<header class="page-header">
								<h1 class="page-title"><?=$genre_title?></h1>
							</header>
							<!-- .page-header -->
							<?php
							$genre_url = "https://api.themoviedb.org/3/discover/".$type."?api_key=".$tmdb_api."&language=en-US&sort_by=popularity.desc&page=".$page."&with_genres=".$genreid;
							if (false !== ($html = @file_get_contents($genre_url))) {	
								$genre_movie_tv = json_decode($html, true);
							} else {
								$curl = curl_init();
								curl_setopt_array($curl, array(
									CURLOPT_URL => $genre_url,
								...
								if ($err) {
								  echo "cURL Error #:" . $err;
									...
									$genre_movie_tv = json_decode($response, true);
								}
							}
							$total_pages = @$genre_movie_tv['total_pages'];
							$total_results = @$genre_movie_tv['total_results'];
								foreach ($genre_movie_tv['results'] as $result) {
									...
									echo '
										<article class="media post-14070" id="post-14070">
											<div class="media-left">
												<a href="//'.$_SERVER['HTTP_HOST'].'/'.$type.'/'.$movie_tv_id.'/'.slugUri($title).'" target="_blank"><img alt="'.$title.'" height="138" src="'.$poster_path.'" width="92"></a>
											</div>


											<div class="media-body">
												<header class="entry-header">
													<h2 class="entry-title"><a href="//'.$_SERVER['HTTP_HOST'].'/'.$type.'/'.$movie_tv_id.'/'.slugUri($title).'" target="_blank" rel="bookmark">'.$title.'</a> <span class="text-color">'.$year.'</span></h2>
							...
							<div class="text-center">
								<nav>
									<ul class="pagination">
										<?php if (!empty($movie_tv_id)) { ?>
											<li class="disabled hidden-xs"><span><span aria-hidden="true">Page <?=$page?> of <?=$total_pages?></span></span></li>
										<?php if (($page==1) || ($page==2)) { ?>
											<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/genre-<?=$type?>/<?=$genreid?>/<?=$_GET['title']?>"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
										<?php } if ($page>=3) { ?>
											<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/genre-<?=$type?>/<?=$genreid?>/<?=$_GET['title']?>/page/<?=$page-1?>"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
										<?php } ?>
										<?php } ?>
										<?php 
										if ($total_results>=20) {
											...
											if ($total_pages >=1 && $page <= $total_pages) {
												...
												$pagination = "";
												if ($page > ($limit/2)) { 
													$pagination .= '<li><a aria-label="First" href="//'.$_SERVER['HTTP_HOST'].'/genre-'.$type.'/'.$genreid.'/'.$_GET['title'].'">1</a></li>
														<li><span><span aria-hidden="true">...</span></span></li>';
												}
												for ($x=$page; $x<=$total_pages;$x++) {
												if ($count < $limit)
													if ($x==1) {
														//$pagination .= "<li><a href=\"//".$_SERVER['HTTP_HOST']."/genre-".$type."/".$genreid."/".$_GET['title']."\">".$x." </a></li>";
														$pagination .= "<li class=\"active\"><span>".$page."<span class=\"sr-only\">(current)</span></span></li>";
													} else if ($x==$page) {
														$pagination .= "<li class=\"active\"><span>".$page."<span class=\"sr-only\">(current)</span></span></li>";
													} else { 
														$pagination .= "<li><a href=\"//".$_SERVER['HTTP_HOST']."/genre-".$type."/".$genreid."/".$_GET['title']."/page/" .$x."\">".$x." </a></li>";
													}
													$count++;
												}
												if ($page < $total_pages - ($limit/2)) { 
													$pagination .= '<li><span><span aria-hidden="true">...</span></span></li><li><a href="//'.$_SERVER['HTTP_HOST'].'/genre-'.$type.'/'.$genreid.'/'.$_GET['title'].'/page/' .$total_pages.'">'.$total_pages.'</a></li><li><a aria-label="Next" href="//'.$_SERVER['HTTP_HOST'].'/genre-'.$type.'/'.$genreid.'/'.$_GET['title'].'/page/'.($page+1).'"><span class="hidden-xs">Next</span> &rsaquo;</a></li>'; 
												}
											}
											echo $pagination;
										}
							...
							<?php if ($type=="movie") { ?>
								...
								<?php
									...
									$now_vote_average = $result['vote_average'];
										...
										$now_count++;
										if (SEO_FRIENDLY=="yes") { 
											$seoFriendlyNow = $_SERVER['HTTP_HOST'] . "/".$type."/".$now_id."/".slugUri($now_title); 
										} else {
											$seoFriendlyNow = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?'.$type.'='.$now_id.'&title='.urlencode($now_title);
										}
									echo '
							...
							<?php } else { ?>
								...
								<?php
									...
									if (SEO_FRIENDLY=="yes") { 
										...
										$seoFriendlyAiring = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?tv='.$airing_id.'&title='.urlencode($airing_title);
									}
									echo '
									<a class="list-group-item clearfix" href="//'.$seoFriendlyAiring.'" rel="bookmark"><img alt="'.$airing_title.'" class="img-responsive pull-left" height="60" src="//image.tmdb.org/t/p/w45'.$airing_poster.'" width="45">
										<div class="list-group-item-details">
