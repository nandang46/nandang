...
<head>
	...
	<title>Popular Movies <?php if ($page>=2) { echo "- Page " . $page . " "; } ?>- <?=TITLE_SITE?></title>
	<meta name="author" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="copyright" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="description" content="Popular Movies <?php if ($page>=2) { echo "page " . $page . " "; } ?>">
				...
				<div class="row">
					<div class="content-area col-md-8" id="primary">
						<main class="site-main" id="main" role="main">
							<div class="row">
								<div class="col-md-12">
									<ul class="breadcrumb" id="crumbs" typeof="BreadcrumbList" vocab="http://schema.org/">
										...
										<li class="active">Popular Movies</li>
									</ul>
								</div>
							</div>
							...
							<!-- .page-header -->
							<?php
							$popular_url = "https://api.themoviedb.org/3/movie/popular?api_key=".$tmdb_api."&language=en-US&page=".$page;
							if (false !== ($html = @file_get_contents($popular_url))) {	
								$pop_movie = json_decode($html, true);
							} else {
								$curl = curl_init();
								curl_setopt_array($curl, array(
									CURLOPT_URL => $popular_url,
								...
								if ($err) {
								  echo "cURL Error #:" . $err;
									...
									$pop_movie = json_decode($response, true);
								}
							}
							$total_pages = $pop_movie['total_pages'];
							$total_results = $pop_movie['total_results'];
								foreach ($pop_movie['results'] as $result) {
									...
									if (SEO_FRIENDLY=="yes") {
										if (SHOW_TITLE_PARAMETER=="yes") { 
											$seoFriendlyPop = $_SERVER['HTTP_HOST'] . "/movie/".$pop_movie_id."/".slugUri($title);
										} else {
											$seoFriendlyPop = $_SERVER['HTTP_HOST'] . "/movie/".$pop_movie_id;
										}
									} else {
										if (SHOW_TITLE_PARAMETER=="yes") {
											$seoFriendlyPop = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?movie='.$pop_movie_id.'&title='.slugUri($title);
										} else {
											$seoFriendlyPop = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?movie='.$pop_movie_id;
										}
									}
									echo '
										<article class="media post-14070" id="post-14070">
											<div class="media-left">
												<a href="//'.$seoFriendlyPop.'" target="_blank"><img alt="'.$title.'" height="138" src="'.$poster_path.'" width="92"></a>
											</div>


											<div class="media-body">
												<header class="entry-header">
													<h2 class="entry-title"><a href="//'.$seoFriendlyPop.'" target="_blank" rel="bookmark">'.$title.'</a> <span class="text-color">'.$year.'</span></h2>
							...
							<div class="text-center">
								<nav>
									<ul class="pagination">
										<?php if (!empty($pop_movie_id)) { ?>
											<li class="disabled hidden-xs"><span><span aria-hidden="true">Page <?=$page?> of <?=$total_pages?></span></span></li>
										<?php if (($page==1) || ($page==2)) { ?>
											<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/upcoming.php"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
										<?php } if ($page>=3) { ?>
											<li><a aria-label="Prev" href="//<?=$_SERVER['HTTP_HOST']?>/upcoming.php?page=<?=$page-1?>"><span class='hidden-xs'>Prev</span> &lsaquo;</a></li>
										<?php } ?>
										<?php } ?>
										<?php 
										if ($total_results>=20) {
											...
											if ($total_pages >=1 && $page <= $total_pages) {
												...
												$pagination = "";
												if ($page > ($limit/2)) { 
													$pagination .= '<li><a aria-label="First" href="//'.$_SERVER['HTTP_HOST'].'/upcoming.php">1</a></li>
												...
												if ($count < $limit)
													if ($x==1) {
														...
														$pagination .= "<li><a href=\"//".$_SERVER['HTTP_HOST']."/upcoming.php?page=".$x."\">".$x." </a></li>";
													}
													$count++;
												}
												if ($page < $total_pages - ($limit/2)) { 
													$pagination .= '<li><span><span aria-hidden="true">...</span></span></li><li><a href="//'.$_SERVER['HTTP_HOST'].'/upcoming.php?page='.$total_pages.'">'.$total_pages.'</a></li><li><a aria-label="Next" href="//'.$_SERVER['HTTP_HOST'].'/upcoming.php?page='.($page+1).'"><span class="hidden-xs">Next</span> &rsaquo;</a></li>'; 
												}
											}
											echo $pagination;
										}
