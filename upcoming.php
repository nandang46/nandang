...
<head>
	...
	<title>Upcoming Movies <?php if ($page>=2) { echo "- Page " . $page . " "; } ?>- <?=TITLE_SITE?></title>
	<meta name="author" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="copyright" content="<?=$_SERVER['HTTP_HOST']?>">
	<meta name="description" content="Upcoming Movies <?php if ($page>=2) { echo "page " . $page . " "; } ?>">
				...
				<div class="row">
					<div class="content-area col-md-8" id="primary">
						<main class="site-main" id="main" role="main">
							<div class="row">
								<div class="col-md-12">
									<ul class="breadcrumb" id="crumbs" typeof="BreadcrumbList" vocab="http://schema.org/">
										...
										<li class="active">Upcoming Movies</li>
									</ul>
								</div>
							</div>
							...
							<!-- .page-header -->
							<?php
							$upcoming_url = "https://api.themoviedb.org/3/movie/upcoming?api_key=".$tmdb_api."&language=en-US&page=".$page;
							if (false !== ($html = @file_get_contents($upcoming_url))) {	
								$up_movie = json_decode($html, true);
							} else {
								$curl = curl_init();
								curl_setopt_array($curl, array(
									CURLOPT_URL => $upcoming_url,
								...
								if ($err) {
								  echo "cURL Error #:" . $err;
									...
									$up_movie = json_decode($response, true);
								}
							}
							$total_pages = $up_movie['total_pages'];
							$total_results = $up_movie['total_results'];
								foreach ($up_movie['results'] as $result) {
									...
									if (SEO_FRIENDLY=="yes") { 
										if (SHOW_TITLE_PARAMETER=="yes") {
											$seoFriendlyUpc = $_SERVER['HTTP_HOST'] . "/movie/".$up_movie_id."/".slugUri($title); 
										} else {
											$seoFriendlyUpc = $_SERVER['HTTP_HOST'] . "/movie/".$up_movie_id; 
										}
									} else {
										if (SHOW_TITLE_PARAMETER=="yes") {
											$seoFriendlyUpc = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?movie='.$up_movie_id.'&title='.slugUri($title);
										} else {
											$seoFriendlyUpc = $_SERVER['HTTP_HOST']."/".FILE_NAME.'?movie='.$up_movie_id;
										}
									}
									...
									echo '
										<article class="media post-14070" id="post-14070">
											<div class="media-left">
												<a href="//'.$seoFriendlyUpc.'" target="_blank"><img alt="'.$title.'" height="138" src="'.$poster_path.'" width="92"></a>
											</div>


											<div class="media-body">
												<header class="entry-header">
													<h2 class="entry-title"><a href="//'.$seoFriendlyUpc.'" target="_blank" rel="bookmark">'.$title.'</a> <span class="text-color">'.$year.'</span></h2>
