...
<head>
	...
	<title>Popular Peoples <?php if ($page>=2) { echo "- Page " . $page; } ?></title>
						...
						<div class="content-area col-md-12" id="primary">
							<main class="site-main" id="main" role="main">
								<ul class="nav nav-tabs">
									<li class="active"><a href="#people"><h3>Popular People</h3></a></li>
								</ul>
								<div class="tab-content">
									<div class="tab-pane fade in active" id="people" role="tabpanel">
										<div id="movie-related" class="row text-center">
										<?php 
										$pop_people_url = "https://api.themoviedb.org/3/person/popular?api_key=".$tmdb_api."&language=en-US&page=".$page;
										$pop_people = json_decode(file_get_contents($pop_people_url), true);
										$total_pages = $pop_people['total_pages'];
										$total_results = $pop_people['total_results'];
										//$tv_count = 0;
											foreach ($pop_people['results'] as $result) {
												$profile_path = isset($result['profile_path']) ? "//image.tmdb.org/t/p/w342".$result['profile_path'] : "//".$_SERVER['HTTP_HOST']."/assets/img/no_poster_available.png";
												$people_id = $result['id'];
												$name = @$result['name'];
												//$tv_count++;
												if (SEO_FRIENDLY=="yes") { 
													$seoFriendlyPeo = $_SERVER['HTTP_HOST'] . "/people/".$people_id."-".slugUri($name); 
												} else {
													$seoFriendlyPeo = $_SERVER['HTTP_HOST'].'/people-bio.php?pid='.$people_id.'&pname='.slugUri($name);
												}
												echo '
													<div class="col-md-3 col-sm-4 col-xs-6">
														<a href="//'.$seoFriendlyPeo.'" target="_blank" data-toggle="tooltip" data-placement="top" title="'.$name.'">
															<img class="img-responsive cover-container" src="'.$profile_path.'" alt="'.$name.'" />
										...
										<?php $page_param = isset($_GET['style']) ? "?style=gallery&page=" : "/people.php?page="; ?>
